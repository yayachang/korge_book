...
class GamePlay : Scene() {
    ...
    override suspend fun Container.sceneInit() {
        ...
        alien = Alien()
        alien.apply {
            load(SharedData.SELECT_RUN_ALIEN)////讀取SharedData.kt的暫存資料
            parentView.addChild(this)
    ...
    fun checkGameOver(){
        if(blood.nowValue == 0){//檢查血包數量
            ...
            SharedData.run {
                ...
                IS_GAME_OVER_SUCCESS = false
            }
            goToGameOver()//進入遊戲結束畫面
        }else if(gameTimer.totalTime == 0){//檢查剩餘時間
            background.stop()
            ItemManager.stop()
            alien.goal()
            SharedData.run {
                GAME_SCORE = score.nowValue
                IS_GAME_OVER_SUCCESS = true
            }
            goToGameOver()//進入遊戲結束畫面
        }
    }
