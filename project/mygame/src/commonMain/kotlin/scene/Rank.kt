...
class Rank : Scene() {
    ...
    override suspend fun Container.sceneInit() {
        ...
        launch {
            //上傳分數
            UserScoreAPI.upload(UserScore(user = "yaya", score = getHighestScore()))

            //下載排行榜資料
            val rankList = UserScoreAPI.getRankList()

            var upperObject = resultString//在至頂的物件參考，我的最高分數image

            var index = 1

            for (data in rankList) {
                val nameString = "$index: ${data.user}" //排名、名稱字串
                val nameBitamp = NativeImage(width = 200, height = 100).apply {
                    getContext2d().fillText(
                        nameString,
                        x = 0,
                        y = fontSize,
                ...
                val rankResultImage = image(nameBitamp) {
                    alignTopToBottomOf(upperObject)
                    alignLeftToLeftOf(upperObject)
                }
                ...
                Score().apply {
                    ...
                    update()
                    alignTopToBottomOf(upperObject)
                    alignLeftToRightOf(rankResultImage)
                }

                upperObject = rankResultImage//換該玩家的名字image當作至頂參考目標
                index ++//名次加1
            }
        }
        ...
        image(resourcesVfs["next.png"].readBitmap()) {
            ...
            alignRightToRightOf(parentView)
